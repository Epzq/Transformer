...
from Transformer import Seq2Seq,Encoder,Decoder,LIN
from Trainer import train,accuracy,accuracyMOC
...
acc_20to50_loader = DataLoader(test_20to50_dataset,batch_size=1,shuffle=False, collate_fn=collate_fn_padd)
...
for epoch in range(N_EPOCHS):
    ...
    train_loss = train(model,train_20to50_loader, optimizer, criterion, duration_loss ,sig, CLIP,batch ,device)
    MOC = accuracyMOC(acc_20to50_loader,model,device,sig,SOS_index,INPUT_DIM)
    print(np.mean(MOC))
